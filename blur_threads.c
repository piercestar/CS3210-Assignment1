...
void *wrapper(void *arguments) {
    struct arg_list *args = arguments;
    unsigned char *src = args->src;
    float *dst = args->dst;
    int width = args-> width;
    int height = args->height;
    float sigma = args->sigma;
    int ksize = args->ksize;
    gaussian_blur(src, dst, width, height, sigma, ksize);
}
...
int main(int argc, char ** argv)
{
    ...
    args.height = height;
    args.sigma = sigma;
    args.ksize = blur_size;
    pthread_create(&thread, NULL, &wrapper, (void *)&args);

    pthread_t thread2;
    struct arg_list args2;
    args2.src = dataB;
    args2.dst = dstB;
    args2.width = width;
    args2.height = height;
    args2.sigma = sigma;
    args2.ksize = blur_size;
    pthread_create(&thread2, NULL, &wrapper, (void *)&args2);
    
    pthread_t thread3;
    struct arg_list args3;
    args3.src = dataG;
    args3.dst = dstG;
    args3.width = width;
    args3.height = height;
    args3.sigma = sigma;
    args3.ksize = blur_size;
    pthread_create(&thread3, NULL, &wrapper, (void *)&args3);
    ...
    printf( "gaussian_blur execution time: %fs\n", (float)(end - start));
